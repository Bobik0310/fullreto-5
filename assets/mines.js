...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-e1cbaa09";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-e1cbaa09">'+'<span class="tile-face-e1cbaa09 tile-front-e1cbaa09"></span>'+'<span class="tile-face-e1cbaa09 tile-back-e1cbaa09"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-e1cbaa09")){return;}
var back=tile.querySelector(".tile-back-e1cbaa09");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-e1cbaa09");if(isMine){tile.classList.add("boom-e1cbaa09");}
if(dim){tile.classList.add("dimmed-e1cbaa09");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-e1cbaa09")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-e1cbaa09");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-e1cbaa09")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-e1cbaa09");void panelEl.offsetWidth;panelEl.classList.add("jolt-e1cbaa09");}
endRound();}
